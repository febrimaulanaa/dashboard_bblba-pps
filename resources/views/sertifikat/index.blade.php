  ...
  <main class="mt-24 pb-20 min-h-[calc(100vh-200px)] flex flex-col justify-center items-center px-6">
    ...
    <div class="bg-surface-container-lowest p-8 md:p-12 rounded-2xl shadow-xl border border-outline-variant/10 w-full max-w-xl relative overflow-hidden">
      ...
      <div class="relative z-10">
        <form action="{{ route('buat') }}" method="post" class="space-y-6">
            {{ csrf_field() }}
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" id="nim" name="nim" placeholder="Masukkan NIM Anda" required
                        class="block w-full pl-12 pr-4 py-4 bg-surface rounded-xl border-outline-variant/50 focus:border-secondary focus:ring focus:ring-secondary/20 transition-all font-body text-on-surface placeholder-outline-variant shadow-inner">
                </div>
            </div>
            
            <button type="submit" class="w-full bg-secondary hover:bg-yellow-600 text-on-secondary font-bold py-4 px-6 rounded-xl transition-all shadow-md active:scale-[0.98] flex items-center justify-center gap-2">
    ...
    <div class="mt-16 flex justify-center">
      <div class="bg-secondary-container/20 border border-secondary/10 px-6 py-4 rounded-full flex items-center gap-4 shadow-sm">
        <div class="w-10 h-10 bg-secondary-container rounded-full flex items-center justify-center text-on-secondary-container">
          <span class="material-symbols-outlined">groups</span>
        </div>
        <div>
          <span class="block text-xs font-bold text-on-surface-variant uppercase tracking-wider">Total Peserta PKBJJ</span>
          <span class="block text-xl font-black text-secondary font-headline">{{ count($usersData) }} <span class="text-sm font-medium text-on-surface-variant">Mahasiswa</span></span>
        </div>
      </div>
    </div>
    @endif
