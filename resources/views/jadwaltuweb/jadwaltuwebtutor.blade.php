...
<head>
    ...
    <title>UT Jakarta - Jadwal Tutorial Tutor</title>
    ...
    <style>
        ...
        .hero-gradient {
            background: linear-gradient(135deg, #385d92 0%, #5275ac 100%);
        }
...
<body class="bg-surface font-body text-on-surface selection:bg-secondary-fixed selection:text-on-secondary-fixed">
    ...
    <nav
        ...
        <div class="flex items-center justify-between px-8 py-4 max-w-[1440px] mx-auto w-full">
            <div class="text-xl font-extrabold tracking-tighter text-[#385d92] dark:text-sky-400 font-headline">
                UT Jakarta — Portal Tutor
            </div>
    ...
    <main class="pt-20">
        ...
        <section class="relative h-[400px] flex items-center overflow-hidden">
            ...
            <div class="relative z-10 max-w-[1440px] mx-auto px-8 w-full text-on-primary">
                ...
                <p class="font-body text-lg text-primary-fixed max-w-xl opacity-90 leading-relaxed">
                    Cek jadwal Tutorial Tatap Muka / Tutorial Webinar (Tuweb) berdasarkan ID Tutor
                </p>
            </div>
        </section>
        ...
        <section class="max-w-[1440px] mx-auto px-8 -mt-16 relative z-20 mb-16">
            <div
                ...
                <div class="flex-1 space-y-2">
                    <h2 class="font-headline text-2xl font-bold text-tertiary">Cek Jadwal Tutorial Tutor</h2>
                    <p class="text-on-surface-variant text-sm">Masukkan ID Tutor Anda untuk melihat
                        jadwal tutorial yang diampuh semester ini.</p>
                </div>
                <div class="w-full md:w-auto flex flex-col md:flex-row gap-4 flex-grow max-w-2xl">
                    <div class="relative flex-grow">
                        <span
                            class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-outline">person_search</span>
                        <input id="data-id"
                            class="w-full pl-12 pr-4 py-4 bg-surface-container-high rounded-xl border-none focus:ring-2 focus:ring-tertiary focus:bg-surface-container-lowest transition-all text-on-surface"
                            placeholder="Masukkan ID Tutor (e.g. 76002278)" type="text" />
                    </div>
                    <button id="get-data"
            ...
            <div id="loading-indicator" class="hidden text-center text-tertiary font-semibold mt-4">
        ...
        <section class="max-w-[1440px] mx-auto px-8 pb-24">
                    ...
                    <table class="w-full academic-table text-left border-collapse min-w-[700px]">
                        <thead>
                            <tr class="bg-surface-container-low border-b border-outline-variant/20">
                                ...
                                <th class="px-8 py-5 font-bold">Kelas</th>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mt-16">
                <div
                    class="md:col-span-2 bg-tertiary-container p-8 rounded-xl text-on-tertiary-container flex flex-col justify-between min-h-[240px]">
                    <div class="max-w-md">
                    ...
                    <div>
                        <a href="https://sl.ut.ac.id/pelayanan_online_utjkt" target="_blank"
                            class="inline-flex items-center gap-2 bg-white text-tertiary-container font-headline font-bold px-6 py-3 rounded-full hover:bg-opacity-90 transition-all shadow-sm focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-tertiary-container">
                ...
                <a href="https://www.ut.ac.id/kalender-akademik/" target="_blank"
                    ...
                    <div>
                        ...
                        <p class="text-on-secondary-container/70 text-sm leading-relaxed mb-0">Pastikan Anda mengetahui
                            jadwal dan tenggat waktu kegiatan akademik terkini.</p>
                    </div>
                </a>
            </div>
        ...
        $(document).ready(function () {
            ...
            $('#get-data').on('click', function () {
                ...
                if (id) {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            if (data && data.length > 0) {
                                ...
                                <tr class="hover:bg-surface-container-low transition-colors group">
                                    <td class="px-8 py-6">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div class="font-bold text-on-surface">${item.nama_mhs || '-'}</div>
                                                <div class="text-xs text-on-surface-variant">NIM: ${item.nim || '-'} <span class="mx-1">•</span> Masa: ${item.masa || '-'}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-8 py-6">
                                        <div class="font-bold text-tertiary">${item.kode_matkul || '-'}</div>
                                    ...
                                    <td class="px-8 py-6">
                                        <div class="flex items-center gap-2 text-on-surface mb-1">
                                            ...
                                            <span class="text-sm font-bold text-tertiary">${item.hari || '-'}, ${item.jam || '-'}</span>
                                        </div>
                                        <div class="text-xs text-on-surface-variant mt-2 bg-surface-container-lowest border border-tertiary/20 rounded-lg p-2.5">
                                            <div class="flex items-start gap-1.5">
                                                ...
                                                <div class="flex-1">
                                                    ...
                                                    <p class="text-[10px] text-on-surface-variant/80 mt-1 leading-snug italic">Tanggal di atas merupakan rentang periode kalender pelaksanaan tutorial secara umum.</p>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                ...
                                $('#no-data-state p').text('Tidak ada jadwal tutorial untuk ID Tutor tersebut. Pastikan ID Tutor yang dicari benar atau jadwal belum keluar.');
                            }
                        },
                        error: function (xhr) {
                            $('#loading-indicator').addClass('hidden');
                    ...
                    $('#error-message').removeClass('hidden').text('ID Tutor tidak boleh kosong. Mohon ketikkan ID Tutor terlebih dahulu.');
                }
            });
        });
    </script>
